...
typealias Push_TypeControllerCallBack = (type:String,detailType:String)->Void
class Push_TypeController: UIViewController,IGLDropDownMenuDelegate {
    ...
    var type = "文学"
    var detailType = "文学"
    var callBack:Push_TypeControllerCallBack?
    ...
    override func viewDidLoad() {
        ...
        segmentLabel = UILabel(frame: CGRectMake((SCREEN_WIDTH-300)/2,20,300,20))
        segmentLabel?.font = UIFont(name: MY_FONT, size: 20)
        segmentLabel?.shadowColor = UIColor.whiteColor()
        segmentLabel?.shadowOffset = CGSize(width: 0,height: 2)
    ...
    func segmentControllerAction(segment:AKSegmentedControl){
        ...
        switch (index){
        case 0:
            self.creatDropDownMenu(self.literatureArray1, array2: self.literatureArray2)
            break
        case 1:
            self.creatDropDownMenu(self.humanitiesArray1, array2: self.humanitiesArray2)
            break
        case 2:
            self.creatDropDownMenu(self.livelihoodArray1, array2: self.livelihoodArray2)
            break
        case 3:
            self.creatDropDownMenu(self.economiesArray1, array2: self.economiesArray2)
            break
        case 4:
            self.creatDropDownMenu(self.technologyArray1, array2: self.technologyArray2)
            break
        case 5:
            self.creatDropDownMenu(self.NetworkArray1, array2: self.NetworkArray2)
            break
        default:
            break
    ...
    func creatDropDownMenu(array1:Array<NSDictionary>,array2:Array<NSDictionary>){
        let dropDownItemArray1 = NSMutableArray()
        for item in array1{
            let item元素 = IGLDropDownItem()
            item元素.text = item["title"] as! String
            dropDownItemArray1.addObject(item元素)
        }
        let dropDownItemArray2 = NSMutableArray()
        for item in array2{
            let item元素 = IGLDropDownItem()
            item元素.text = item["title"] as! String
            dropDownItemArray2.addObject(item元素)
        }
        ...
        self.dropDownMenu1?.paddingLeft = 15 //缩进
        self.dropDownMenu1?.delegate = self
        self.dropDownMenu1?.type = .Stack//动画类型
        self.dropDownMenu1?.itemAnimationDelay = 0.1//动画间隔
        self.dropDownMenu1?.gutterY = 5//每个itemY轴间隔
        self.dropDownMenu1?.dropDownItems = dropDownItemArray1 as [AnyObject]
        ...
        self.dropDownMenu2?.dropDownItems = dropDownItemArray2 as [AnyObject]
